// Repo hygiene guard: no file may start with a UTF-8 BOM — tracked or not.
...
// The BOM was introduced by a working-tree edit, so scanning only tracked
...
for (const file of listed) {
  ...
  try {
    ...
    continue // symlinks and other oddities are not BOM concerns
  }
...
if (offenders.length > 0) {
  console.error('BOM guard failed — these files start with EF BB BF:')
  for (const f of offenders) console.error('  ' + f)
  process.exit(1)
}
console.log(`BOM guard: ${listed.length} files (tracked + untracked) clean`)
